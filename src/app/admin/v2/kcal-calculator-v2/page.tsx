...
export default function KcalCalculatorV2Page() {
  ...
  const calculate = useCallback((autoCalculate = false) => {
    ...
    // Macronutrients
    // Proteins: 1.6 g/Kg / 2.2 g/Kg (men)
    const proteinPerKg = formData.gender === 'male' ? 2.2 : 1.6;
    const protein = Math.round(weight * proteinPerKg);

    // Fats: MINIMUM 0.5-1 g/Kg (using 0.8 as middle)
    const fat = Math.round(weight * 0.8);

    // Carbohydrates: MINIMUM 2-6 g/Kg (remaining calories)
    const proteinCalories = protein * 4;
    const fatCalories = fat * 9;
    const carbCalories = finalCalories - proteinCalories - fatCalories;
    const carbs = Math.max(Math.round(weight * 2), Math.round(carbCalories / 4)); // Minimum 2g/Kg
    ...
    setShowResults(true);
  }, [formData.weight, formData.age, formData.gender, formData.bodyType, formData.objective, formData.objectivePercentage, formData.dailyActivity, formData.trainingHours, formData.trainingIntensity]);
  ...
  }, [formData.weight, formData.age, formData.gender, formData.bodyType, formData.objective, formData.objectivePercentage, formData.dailyActivity, formData.trainingHours, formData.trainingIntensity, calculate]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          router.push('/admin/v2/nutrition-calculations');
        }, 1500);
      } else {
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="bg-red-50 p-4 rounded-lg border border-red-200">
                      ...
                      <div className="text-xs text-red-600 mt-1">
                        {formData.gender === 'male' ? '2.2 g/Kg' : '1.6 g/Kg'}
                      </div>
                    </div>
