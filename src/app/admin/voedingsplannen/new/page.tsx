  ...
  const handleCreatePlan = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // If calculation has a customerId, automatically assign the plan to the customer
        if (calculation?.customerId) {
          try {
            const assignResponse = await fetch('/api/customer-nutrition-plans', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
              body: JSON.stringify({
                ...
                status: 'active',
                notes: `Created from calculation on ${new Date().toLocaleDateString()}`
              }),
            });
            
            if (!assignResponse.ok) {
              const errorData = await assignResponse.json();
              console.warn('Failed to assign plan to customer:', errorData.error || errorData.details || 'Unknown error');
              console.warn('Assignment error details:', errorData);
              // Show alert to user but don't block
              alert(`Plan created successfully, but assignment failed: ${errorData.details || errorData.error || 'Unknown error'}`);
            } else {
              console.log('Plan successfully assigned to customer');
            }
          } catch (assignError) {
            console.error('Error assigning plan to customer:', assignError);
            // Don't block navigation if assignment fails - plan is still created
            alert('Plan created successfully, but assignment failed due to network error');
          }
        }
        
        router.push(`/admin/voedingsplannen/${newPlan.id}`);
      } else {
